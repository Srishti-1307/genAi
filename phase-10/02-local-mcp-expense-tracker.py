...
# Create & return a connection to sqlite database. If dbfile doesn't exist, SQlite will create it automatically. 
def get_db_connection():
    conn = sqlite3.connect(DB_FILE)
    return conn


# Initializing database, creating table if does not exist 
def init_db():
...
""", (date, amount, category, sub_category, note))
    conn.commit()
    conn.close()
    return f"Expense amount {amount} added in {category} category"
